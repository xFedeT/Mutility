package it.fedet.mutility.velocity.common.server.config.value;
...
public abstract class ConfigValue<T> implements Supplier<T> {
    ...
    /*public static void loadValues(Class<?> clazz, FileConfiguration config) {
        ...
        for (Field field : fields) {
            ...
            try {
                ...
                ChatLogger.error("Failure to load configuration values of class " + clazz.getName(), exception);
            }
        }
    }
    ...
    protected abstract void setToConfig(FileConfiguration config, T value);
